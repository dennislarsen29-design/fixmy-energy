...
### Dialer Script + Call Flow panel (2026-07-23)
Embedded in the current-call card of the Black Box Dialer (`bbScriptPanelHtml(lead)` in portal.html, rendered right under the Call/Text buttons — both admin and tech/sales see it since they share `renderBBDialerView`). Collapsible "📜 Call Script" panel (open state in localStorage `bbScriptOpen`), stage chips: **🗺 Flow** (visual in-call decision flow mapped to the exact disposition buttons + no-answer contact cadence: attempt 1 no VM → attempt 2 VM+text → attempt 3 different time → rest ~30 days) then **Opener / Hook / Qualify / Book It / Objections / Voicemail / Callback**. Script text auto-personalizes per lead: homeowner first name, defunct `original_installer` (the orphaned-system hook), SDCP-territory tip, lead address in the booking confirm, and the rep's own name via `bbRepName()`. Stage switching (`bbScriptSetStage`) and collapse are **DOM-local updates only** — a full `renderBBDialerView()` would wipe the rep's typed note and any live AI transcript. Compliance built into the script: DNC is scripted + labeled mandatory on "take me off your list."
