...
### Dual-profile switcher — Admin ⇄ Tech (2026-07-31)
Dennis wears two hats: owner/**Admin** and field **Tech** (`tech4`). Swapping used to mean signing out and back in under a different code, which also threw away the 30-day session and all in-page context. Both identities now ride in one session and swap from a pill in the dash header (`#profileSwitch`).
- **Security model (deliberate, do not "simplify"):** **Admin → Tech is a de-escalation** (strictly less access) so it needs **no re-auth**. **Tech → Admin is an escalation**, so it costs the `ADMIN_CODE` **once**; the profile then stays unlocked until an explicit Sign Out. Admin logging in gets the owner's tech profile attached for free; the owner-tech logging in sees Admin offered with a 🔒 until unlocked.
- **Who sees it:** only the owner. `OWNER_TECH_ID = 'tech4'` (declared next to `TECHS`) is the single knob linking the Admin profile to a tech identity. Every other role (`tech2`/`tech3`/`tech5`, rep, setter, ops, customer) calls `_profileSeed` → `_clearProfiles()`, so the switcher never renders for them.
- **State:** localStorage key `fmProfiles` = `{active, admin:{unlocked}, tech:{id,name,email}, offerAdmin, adminView}`, separate from `fmUser`. `signOut()` clears it.
- **Context preservation:** the admin tab is remembered in `fmProfiles.adminView` and reopened by `_adminRestoreView()` (dispatch map `_ADMIN_VIEW_FNS` — only `dashboard` goes through `renderAdminView`; every other admin tab is its own top-level render fn). Unknown or throwing tabs fall back to Leads, never a blank pane. The Tech side needed nothing — it already persists `salesView_<id>` in sessionStorage.
- Functions: `_profileState` / `_saveProfileState` / `_clearProfiles` / `_ownerTech` / `_profileSeed` / `_profileRegister` / `_renderProfileSwitcher` / `window.switchProfile` / `_adminRestoreView`, all in portal.html next to the auth helpers. `_renderProfileSwitcher()` is called from `loadAdminDashboard` and `loadSalesDashboard` right after `#userName` is set.
- Verified: 37-assertion harness against the extracted real source (`scratchpad/test-profile-switch.js`) covering the escalation rules, wrong-code/cancel paths, per-role visibility, tab restore + fallback, and sign-out; plus a Playwright run of the real portal confirming the pill renders and round-trips Admin → Tech → Admin without re-prompting.

## iOS App Conversion (planned 2026-07-31 — build in progress)
Turning the portal into a real App Store app, **staff-only** (Admin/Setter/Tech). Ops + Customer views are explicitly out of scope for this phase.

**Approach: Capacitor wrapper + native polish.** The web assets (`portal.html` and friends) are bundled inside the app; **Supabase, all 69 Netlify functions, GHL, and the auth model are untouched** — zero backend rework. This is the only approach where "no capability lost" is true by construction (portal.html is ~18k lines / ~365 functions; a native or RN rebuild would be a re-implementation project and the expensive path). A *bare* wrapper is what Apple rejects under **Guideline 4.2 (minimum functionality)** — the native layer below is approval insurance, not decoration.

**Codebase-specific gotchas that MUST be handled when wrapping:**
- **`webkitSpeechRecognition` does not exist in WKWebView** — the dialer's 🎙 AI Notes (6 usages) will silently break. Needs `@capacitor-community/speech-recognition` behind an adapter shim that falls back to the web API in a browser. Highest-priority native shim; the dialer is a core rep workflow.
- **Auth gets *better*, not just preserved:** move `fmUser`/`fmProfiles` into Capacitor **Preferences** (real native storage). WKWebView can evict localStorage under storage pressure; Preferences makes sessions effectively permanent until explicit Sign Out.
- `sms:` (11) and `tel:` (17) links work natively in the wrapper — no changes needed.
- Photo upload gets the native iOS camera/picker via Capacitor Camera, feeding the same `uploadPhotos()` path (already non-blocking since the 2026-07-22 async-Quoya fix).
- **Update flow decision (open):** today Netlify deploys reach everyone instantly; a bundled app freezes web assets at submission. Options: resubmit per change (free, slow), a live-update service (e.g. Capgo, ~$15/mo, Apple-compliant for web content, keeps the ship-daily workflow), or point at the live URL (raises 4.2 rejection risk — not recommended). Leaning bundle + Capgo, deferred until after first approval.
- Apple review prep: reviewer demo login, privacy policy URL, App Privacy labels (customer PII is collected). Account-deletion rule doesn't apply (staff-provisioned accounts). IAP rules don't apply — customer payments live on `sign.html`, deliberately outside the app.

**Cost shape:** $99/yr Apple Developer (enroll as **Solar Review Corp**, org enrollment needs a free D-U-N-S — 1–3 week lead time, start early), $0–15/mo services, no Mac purchase required (GitHub Actions macOS runners or Codemagic + fastlane can build/sign/ship to TestFlight).

**Build chunks (one session each, independently testable so a failure never invalidates earlier work):**
0a ✅ Admin⇄Tech profile switcher (web-only, shipped) · 0b auth-storage abstraction + mobile-nav polish (web-only) · 1 Capacitor scaffold · 2 native shims (speech/camera/clipboard/Preferences/safe-area) · 3 push notifications end-to-end (`device_tokens` table + sender function) · 4 iPad layout pass · 5 CI/CD to TestFlight · 6 store assets + submission.
...
- **Brand decision (2026-07-04, per Dennis — FULL REBRAND):** The customer-facing brand is **Solar Review** everywhere. FixMy.Energy is dissolved as a brand name across all public pages (index, check-preview, careers, sign, meet, onboarding, customer-facing portal strings). Keep: `fixmy.energy` wherever it is the literal domain/URL/email (still the live domain), `lead_category='fixmy'` data values, `isFixMySave`-style internal identifiers, internal admin tab labels ("FixMy" = internal name for the diagnostic/battery business line — rename only if Dennis picks a new line name), and the rep agreement "Solar Review Corp (DBA: FIXMy.Energy)" legal text (contract language — change only when the DBA filing itself changes). Schema keeps `alternateName: "FixMy.Energy"` for old-name searches.
  - ⚠️ **EXCEPTION — the iOS app is branded `FixMyEnergy` (2026-07-31, per Dennis).** The App Store listing, app name, home-screen display name, icon, and the app's login screen all say **FixMyEnergy** (one word, no space/dot). This does NOT reverse the rebrand: the app is **staff-only** (Admin/Setter/Tech), so it is not customer-facing, and the name matches the domain reps already type. Everything past the app's login screen keeps its existing Solar Review styling. Do **not** "fix" the app name to Solar Review per the rebrand SOP — this exception is intentional. Note the App Store *seller* name is fixed to the Developer Program entity ("Solar Review Corp") and is not editable per-app; that pairing is fine given the legal DBA is "FIXMy.Energy".
