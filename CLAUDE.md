...
### High Priority
- **17 Indeed applicants imported as a seed, not yet applied (2026-07-17):** The "Team & Hiring → Hiring Pipeline" sub-tab and its `candidates` table already existed (fed today only by the public `/careers` page → `careers-apply.js`). 17 unread-in-spirit (already-marked-read, never actioned) Indeed "[Action required] New application" emails for **Solar Energy Consultant — Diagnostics & Battery Storage Sales** (applied 2026-05-29 to 2026-05-31, sitting untouched for ~7 weeks) were found in info@fixmy.energy and staged as `supabase/seed/indeed_candidates_2026-05.sql` — ⚠️ **not yet run** (direct Supabase access was policy-blocked in the build session, same blocker as other pending seeds in this file). Run it in the SQL Editor or via MCP, then it'll show under Hiring Pipeline with `source='indeed'`, `status='applied'`. Indeed's notification emails don't expose a real email/phone/resume/cover-letter — only self-reported qualification badges (Driver's License, US work authorization, Leadership, Sales) and a "View resume" link that requires logging into the Indeed employer dashboard directly; contact info and resumes for these 17 must be pulled from there, not from this portal. **Strongest fits by badge coverage** (all 4 badges — DL + work auth + Leadership + Sales, most relevant for a driving/field sales role): Alwin Jones, Manulito Loman, Ivan Yalda, Robert Buller, Victor Franchetti, Carson Pugh, Brett Banaszak. Next tier (missing only Driver's License, worth asking about in a phone screen since the role requires driving to customer homes): Caitlin McLeod, Ukiah Dublinski. Weakest signal (bare "US work authorization" only, no Sales/Leadership/DL badge at all): Fred Havens, Michael Mcglone, Robert Shelton III.
