...
- **Customer photos on the advice card (2026-07-21):** each advice lead's uploaded job photos are fetched (`job_photos` table `.or(job_id/customer_id)`, storage-bucket fallback) and rendered as a horizontal thumbnail strip (tap = open full-size) so the partner can eyeball the equipment/site while advising. These are the same customer-visible photos.
