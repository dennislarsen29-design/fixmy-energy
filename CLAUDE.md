...
- **Migration `20260803_setter_database.sql`** (⚠️ **not yet applied** — run in the SQL Editor): `rep_shifts` (shift clock), `one_on_one_notes` (durable 1:1 record), plus `team_members.onboarded_at / gusto_status / gusto_sent_at / start_date`. Anon-key RLS, same trust model as `lead_activity`/`coaching_reports`. Everything degrades silently if unapplied — dialing is never blocked.
...
- **Migrations:** `20260723_agent_reports_emailed.sql` (adds `emailed_at` to `agent_reports`) + `20260723_coach_reports_emailed.sql` (adds `emailed_at` to `personal_coach_reports`). ⚠️ Run both in the SQL Editor.
...
- **Sub Sheet (Rate Sheet):** a "Reroofing → Reroof (per square) $750.00 / sq" row was added (1 square = 100 sq ft; dealer cost, retail set per proposal).
...
- ⚠️ Run `20260730_rep_agreement_version.sql` in the SQL Editor — it's non-destructive (`add column if not exists`), existing rows land `NULL` which correctly fails the version check and triggers re-sign.
...
- New `job_photos` columns (migration `20260722_quoya_async_sync.sql`, ⚠️ **also not yet applied — depends on the schema fix above running first**): `quoya_status` (pending|done|failed|skipped), `quoya_attempts`, `quoya_synced_at`, `expected_label`.
