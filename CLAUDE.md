...
- **Dialer/Setter split (2026-07-27):** the 40%-of-net pool splits **20% closer / 20% setter** when a **setter (dialer) set the deal and differs from the closer**; self-generated (setter == closer, or no setter) → closer keeps the full **40%**. Dialer = the setter role (reuses `customers.setter_name`); a Tech can dial, a setter-only can't close. **Dennis (tech4) as closer earns $0 regardless, but the setter's 20% still pays.** The dialer's "Booked!" (`bbDialCommit`) stamps `setter_name` = the dialing rep; admin reassigning a different closer (inline Tech dropdown on the card) is what triggers the split. Computed + locked in from the FixMy job editor's accounting panel: **`edRecordFixmySplit(cid)`** recomputes net authoritatively at click time and inserts a `rep_commission` (closer, 20%/40%) and a **`setter_commission`** (setter, 20%; new `commissions.kind`, no migration — `kind` is free text) row, both `sold`→`paid`. Both book to P&L **COGS "Incentives - Sales"** (`finComputePnl`/`finMonthlySeries`) and show per-payee in the Finance **Commissions** sub-tab (`kindLbl.setter_commission='setter split'`). Worked examples (all verified): PW3 $13,950−$12,500 = $1,450 net → full-40% ref $580, setter $290; PW3 avg net $4,000 → setter $800; Diagnostic $699−$300 = $399 → setter $79.80.
