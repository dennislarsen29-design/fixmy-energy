...
### Dialer AI Transcript / Note-Taker (2026-07-23)
"🎙 AI Notes" button in the dialer's Call row. Rep taps it, puts the call on speakerphone, and the browser's `SpeechRecognition` (webkit prefix; iOS Safari 14.5+/Chrome) transcribes both sides live into a purple transcript panel (`#bbAiPanel`), auto-restarting on the silence-timeout `onend`. Tapping **⏹ Stop & Summarize** posts the transcript + lead context (name, installer, address) to **`netlify/functions/dialer-notes.js`** — hardened like claude-vision.js (origin allowlist, payload reconstructed, transcript capped 12k chars), model `claude-haiku-4-5-20251001`, ~300 tokens — which returns `{note, suggested_outcome}`. The note (prefixed 🎙) **prefills `#bbDialNote`** (appends to anything the rep typed) and the suggested disposition shows as a colored hint next to the status — it is deliberately **never auto-committed**; the rep taps the real disposition button, which saves the note through the normal `bbLogActivity` path (notes feed + lead_activity). Unsupported browser or denied mic degrades to a friendly message; short transcripts (<25 chars) are skipped rather than summarized into noise.
