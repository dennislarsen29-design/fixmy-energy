...
- **Part D — broadened deposit review categories (`finRenderDepositReviewQueue()`/`finClassifyDeposit()`):** per-deposit buttons **Top Tier / New Solar / Other provider** (→ `commissions` override row, line `top_tier`/`new_solar`/`fixmy`, payee `solar_review_corp`, status `paid` — feeds P&L Commission Income), **Other Income** (→ override row line `other`, payee_name "Other Income" — `finComputePnl()` routes line `other` to the "Other Income" account, distinct from Commission Income), **Travel Reimb.** (→ NEGATIVE `expense_transactions` row on "Travel Expense", contra-expense, `dedupe_hash 'depclass_'+id`), **Owner Transfer** (marked confirmed, NOT revenue, excluded from P&L), **Ignore** (status `ignored`).
