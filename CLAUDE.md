...
- **Financial Coach sub-tab (`renderCoachView()`):** Phase 3 — AI advisor (net-worth dashboard + 1/5/10yr projections, strategy engine, accountability, vision board, consultant chat). ⚠️ Placeholder until Phase 3 lands.
- **⚠️ Migration `20260714_personal_finance.sql` not yet applied** (Supabase MCP can't self-approve in this session env). Run it in the SQL editor; the Bookkeeper degrades gracefully with a banner pointing at the file until then. Optional but recommended: set `PERSONAL_ACCESS_KEY` in Netlify to lock the personal data behind a key.
