...
exports.handler = async function (event) {
  ...
  const prompt = [
    ...
    '  "guidance": "<ONE short, plain-spoken sentence of ACTIVE coaching for the rep RIGHT NOW, or null if there is genuinely nothing useful to add this pass. Be proactive — do not wait until something is clearly broken before speaking up. Good moments to coach: a required field is still missing and the moment to ask has passed; the conversation has drifted off-topic; the homeowner just raised a real pain point (a high bill, their installer going out of business, no one monitoring the system, a recent outage) worth reinforcing right now, not just noting; qualifying sounds complete and it is time to move toward booking the free evaluation; the call sounds like it is winding down with no appointment actually set. Ground every suggestion in these standing rules and never contradict them: there is NO no-show fee (always free to reschedule), never claim a manufacturer affiliation/partnership with SolarEdge, Enphase, or any equipment maker, incentives may be alluded to in general terms but a specific program name or dollar figure must never be stated over the phone, never talk down the homeowner\'s original installer, and before ending the call the rep must confirm BOTH a real date AND the two-hour arrival window out loud. Still return null when the call is going fine and there is nothing worth interrupting for — the goal is genuinely useful, well-timed coaching, not a running commentary.>"',
    '}',
    '',
