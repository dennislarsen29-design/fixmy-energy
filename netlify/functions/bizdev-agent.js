...
exports.handler = async function() {
  ...
  try {
    ...
    // Forced write phase: if Claude analyzed data but never called write_action_item
    if (actionItemCount === 0 && messages.length > 2) {
      console.log('[bizdev-agent] No items written — forcing write phase');
      messages.push({ role: 'user', content: 'You have all the data. Now call write_action_item for each finding — one call per action item. Do not respond with text.' });
